...
The charming section has arrived! Here is given a simple and useful function to generate the list of possible parameters for an estimator.
\begin{python}
...
\begin{itemize}
	...
	\item \textbf{Max Depth} -- The maximum depth of the tree. If None, then nodes are expanded until all leaves are pure or until all leaves contain less than Min Samples Split samples.
	\item \textbf{Max Features} -- The number of features to consider when looking for the best split. It can obtain values of ``auto", ``sqrt" and ``log2".
	\item \textbf{Max Leaf Nodes} -- Grow a tree with Max Leaf Nodes in best-first fashion.
	\item \textbf{Min Impurity Decrease} -- A node will be split if this split induces a decrease of the impurity greater than or equal to this value.
	\item \textbf{Min Impurity Split} -- Threshold for early stopping in tree growth. A node will split if its impurity is above the threshold, otherwise it is a leaf.
	...
	\item \textbf{Min Weight Fraction Leaf} -- The minimum weighted fraction of the sum total of weights (of all the input samples) required to be at a leaf node. Samples have equal weight when Sample Weight is not provided.
	...
	\item \textbf{Random State} -- The seed used by the random number generator.
